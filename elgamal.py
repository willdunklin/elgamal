...
class ElGamal:
    ...
    def encrypt_text(self, text: str) -> list[tuple[int]]:
        b = text.encode('utf-8') # byte string representing text
        bytes_per_n = max(self.k // 8, 1)

        # split text into chunks of k bits and convert them to integers
        ints = []
        for i in range(math.ceil(len(b) / bytes_per_n)):
            byte_substr = b[i*bytes_per_n : (i+1)*bytes_per_n]
            ints.append(int(byte_substr.hex(), base=16))

        # encrypt the integer representation of text
        return [self.encrypt(i) for i in ints]

    def decrypt_text(self, ints: list[tuple[int]]) -> str:
        text = ''

        for (x, b) in ints:
            x = self.decrypt(x, b)
            num_bytes = math.ceil(math.log(x)/math.log(256)) if x != 1 else 1
            text += x.to_bytes(num_bytes, 'big').decode('utf-8')
